document.addEventListener('DOMContentLoaded', function() {
  ...
  [...document.querySelectorAll(".case-study_item")];
    ...
    function ScrollMarquee() {
      ...
      tl2.fromTo(".case-study_group", { "--progress": 0 }, {
  "--progress": 1, ease: "none" });
    ...
    tl3.fromTo(".cms-wrapper", { opacity: 0 }, { opacity: 1, delay:
   0.3 });
    tl3.fromTo(".case-study_group", { "--progress": 0.2 }, {
  ...
  zIndex: 100 });
    ...
    function showModal() {
      ...
      setTimeout(() => {
        item = document.querySelector(".is-active
  .case-study_card");
        flipInto(modalTarget);
      }, 1000);

      const split =
  SplitText.create(document.querySelectorAll(".prompt_result > *"),
   {
        type: "words, chars",
        charsClass: "char",
      });
      ...
      tl.fromTo(".modal_content_wrap", { delay: 0.4, "--progress":
  1 }, { "--progress": 0, ease: "power1.inOut", duration: 0.6 });
      tl.fromTo(".panel", { width: "0rem" }, { width: "40rem",
  ease: "power1.inOut", duration: 0.6 }, "<");
      tl.fromTo(".modal_backdrop", { opacity: 0 }, { delay: 0.8,
  opacity: 1, duration: 0.2 });
      tl.from(".char", { opacity: 0, stagger: { each: 0.005, ease:
  "power1.out" }, duration: 0.005 });
    }
  ...
  flipInto(document.querySelector(".case-study_item.is-active"));
      let tl = gsap.timeline();
      tl.fromTo(".modal_backdrop", { opacity: 1 }, { opacity: 0,
  duration: 0.2 });
      tl.fromTo(".modal_content_wrap", { "--progress": 0 }, {
  delay: 0.6, "--progress": 1, ease: "power1.inOut", duration: 0.6
  });
      tl.to(".panel", { width: "0rem", ease: "power1.inOut",
  duration: 0.6 }, "<");
      tl.set(".modal_wrap", { display: "none" });
      tl.to(".prompt_wrap", { yPercent: 0, scale: 1, ease:
  "power2.in" });
      promptInput.value = "";
    }
  ...
  "https://cary-blog-runtime.dry-forest-5ebe.workers.dev";
    ...
    async function handlePrompt(userPrompt) {
      ...
      gsap.to(".prompt_wrap", { yPercent: 100, scale: 0.6,
  ...
  from worker");
        ...
        caseStudyItems.forEach(function(el) {
          const itemTitle = el.querySelector(".case-study_title")
  && el.querySelector(".case-study_title").textContent.trim();
          el.classList.toggle("is-active", itemTitle ===
  data.title);
        });
  ...
  data.link : window.location.origin + data.link;
        ...
        promptResult.innerHTML = "<p>Sorry, something went wrong.
  Please try again.</p>";
      }
    }
    ...
    document.querySelector(".prompt_circle").addEventListener("clic
  k", function() {
      const userPrompt = promptInput.value.trim();
      if (userPrompt) handlePrompt(userPrompt);
    });

    document.querySelectorAll(".modal_close_wrap,
